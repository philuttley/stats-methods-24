...
**Acceptance Criterion**: Calculate the acceptance probability $$\alpha$$ given by $$\alpha(x'|x_i) = {\rm min} \left(1,\frac{p(x')q(x_i|x')}{p(x_i)q(x'|x_i)}\right)$$, where $$p(x)$$ is the target distribution we want to sample from.
